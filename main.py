...
# --- БЛОК САМОУСТАНОВКИ (Исправляет твою ошибку) ---
...
# Railway предоставляет нам папку /app. Если мы пишем туда, файлы сохраняются.
...
async def admin_upload(client, message):
    ...
    conn.commit()
    conn.close()
    await message.reply(f"✅ Ссылка: https://t.me/plutoniumfilesBot?start={f_hash}")
